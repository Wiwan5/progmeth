	...
	private void addCounter(Field field) {
		 ...
		 for (int i = 0; i < field.getRow(); i++) {
		 	for (int j = 0; j < field.getCol(); j++) {
		 		if (field.getField()[i][j] == 1) addNewObject(new Shelf(j*size+size/2+100, (i+1)*size+120, 70,70,null));
		 		...
		 		else if (field.getField()[i][j] == 9) addNewObject(new Shelf(j*size+size/2+100, (i+1)*size+120, 70,70,new Plate()));
