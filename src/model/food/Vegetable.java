	...
	public void draw(GraphicsContext gc, double x, double y) {
		...
		if (state == CAN_CHOP) {
			if (timeToChopped >= 55 && timeToChopped <= 70) {
				gc.drawImage(ResourseLoader.veg[6], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 50 && timeToChopped < 55) {
				gc.drawImage(ResourseLoader.veg[5], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 40 && timeToChopped < 50) {
				gc.drawImage(ResourseLoader.veg[4], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 30 && timeToChopped < 40) {
				gc.drawImage(ResourseLoader.veg[3], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 20 && timeToChopped < 30) {
				gc.drawImage(ResourseLoader.veg[2], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 10 && timeToChopped < 20) {
				gc.drawImage(ResourseLoader.veg[1], x - 30, y - 60, 55, 50);
			} else if (timeToChopped >= 0 && timeToChopped < 10) {
				gc.drawImage(ResourseLoader.veg[0], x - 30, y - 60, 55, 50);
			...
			gc.drawImage(ResourseLoader.veg[7], x - 25, y - 60, 50, 45);
		}
	}
